...
import (
	...
	//"github.com/ggarrido85/api-backend/api/middleware"
...
func corsOption(ctx context.Context, conf Configuration) cors.Config {
	...
	for i, s := range []string{conf.AppUrl, conf.BackofficeUrl} {
...
func InitRouterMiddlewares(
	...
	telemetryRessources string infra.TelemetryRessources*/
) *gin.Engine {
	if conf.Env != "development" {
