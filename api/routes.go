...
import (
	...
	// "github.com/ggarrido85/api-backend/pubapi"
	// pubapiv1 "github.com/ggarrido85/api-backend/pubapi/v1"
...
func addRoutes(r *gin.Engine, conf Configuration, tokenHandler string, logger *slog.Logger) {
	// Resuqest timeout
	tom := timeoutMiddleware(conf.DefaultTimeout)
	parsedAppUrl, err := url.Parse(conf.AppUrl)
	if err != nil || parsedAppUrl.Scheme == "" || parsedAppUrl.Host == "" {
		logger.Error("Failed to parse the Marble app URL environment variable. The decision page url passed in the decisions API response will be empty.", "url", conf.AppUrl)
	}
	...
	r.GET("/signup-status", tom, handleSignupStatus())
